...
int8_t os_fOpen(uint8_t read, st_encoder_t * p_enc)
{
    ...
    E4C_TRY {
        if (read) {
            p_enc->p_fp = CreateFile(TEXT(p_enc->path), GENERIC_READ, 0, NULL,
                               OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

            if (INVALID_FILE_HANDLE == p_enc->p_fp) {
                E4C_THROW(RuntimeException, "Failed to associate a descriptor"
                                                            "with opened file.\n");
            }

            if (!GetFileSizeEx(p_enc->p_fp, &p_enc->fsize)) {
...
int8_t os_fOffset(FILE* p_fp, uint32_t off)
{
    int8_t err = 0;
                    ...
                    st_encArgs_t* threadArgs, uint16_t maxElems)
{
    ...
    if ((dirDesc = opendir (dirPath)) == NULL) {
        ...
        while ((dirFile = readdir(dirDesc)) && (dirSize < maxElems) )
        {
            ...
            threadArgs[dirSize].p_fname = strdup(dirFile->d_name);
